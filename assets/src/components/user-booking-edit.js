...
function UserBookingEdit({config, booking, onBookingChanged, close}) {
  ...
  useEffect(() => {
    if (booking) {
      ...
      Api.fetchResource(config.api_endpoint, booking.resourceMail).then(
        (resource) => {
          setResources([resource]);

          Api.fetchEvents(config.api_endpoint, [resource], date)
   ...
   * @param newEnd
   */
  const requestDateChange = (booking, newStart, newEnd) => {
    const newData = {
      id: booking.id,
